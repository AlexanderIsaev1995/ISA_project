...
public class OrderDtoServiceTest {
    ...
    public void AddOrderDtoSetUpTest() throws Exception {
        ...
        orders.add(order);
        orders.add(orderOne);
        OrderDtoService orderDtoService = OrderDtoService.from(order);
        OrderDtoService orderDtoServiceOne = OrderDtoService.from(order,TestUtil.operations());
        List<OrderDtoService> orderDtoServices = OrderDtoService.from(orders);

        assertNotNull(orderDtoService);
        assertNotNull(orderDtoServiceOne);
        assertNotNull(orderDtoServices);
    }
}
